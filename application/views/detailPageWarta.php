...
<body>
    ...
    <div class="row">
        ...
        <div class="col-md-8">
        <!-- Judul -->
        <h1 class="header-title"><?= $warta->judul ?></h1>
        ...
        <div class="d-flex align-items-center mt-3 mb-3">
           <button class="btn btn-custom-back me-2 small-button" onclick="window.location.href='<?php echo site_url('pageJournal'); ?>'">
            ...
            <button class="btn btn-custom-views me-2 small-button">
                <i class="fas fa-eye"></i> <?= $warta->views ?> views
            </button>
        </div>
        <hr class="horizontal-line"/>
        <!-- Tentang Penulis -->
        <h2 class="header-subtitle" style="margin-top: 20px;">Redaksi</h2>
            <div style="margin-left: 35px; font-size: 14px; margin-top: 10px;">
                <p> <?= $warta->penyusun ?></p>
            </div>
        
        <!-- Garis Besar Jurnal -->
        <h2 class="header-subtitle">Deskripsi</h2>
        ...
        <!-- PDF Viewer -->
         <h2 class="header-subtitle">Pratinjau E-Warta</h2>
        <div style="margin: 0px 35px; margin-bottom: 35px;">
            <iframe 
                src="<?= base_url('uploads/' . $warta->file_name) ?>" 
                width="90%" 
                height="600px" 
        ...
        <div class="author-section">
            ...
            src="<?= base_url('uploads/' . $warta->file_thumbnail) ?>" 
...
<script>
  ...
  button.addEventListener('click', function() {
    var wartaId = this.dataset.id;
    document.getElementById('modalDownload').style.display = 'flex';
    document.getElementById('btnKirim').setAttribute('data-id', wartaId);
  });
});
...
document.getElementById('btnKirim').addEventListener('click', function() {
  ...
  var wartaId = this.getAttribute('data-id');
  ...
  fetch("<?= base_url('index.php/warta/download') ?>", {
    method: 'POST',
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded',
    },
    body: `warta_id=${wartaId}&email=${encodeURIComponent(email)}`
  })
  .then(res => res.text())
  .then(res => {
    console.log(res);
    document.getElementById('modalDownload').style.display = 'none';
    window.location.href = "<?= base_url('index.php/warta/download_file/') ?>" + wartaId;
    setTimeout(() => {
    location.reload();
    }, 300);
  });
