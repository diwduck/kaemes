                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-primary">
                        ...
                        <?php 
                            ...
                            {?>
                                <h5> <?php echo $this->session->flashdata('msg_berhasil');?></h5>
                      <?php }
                            elseif ($this->session->flashdata('msg_berhasil_upload')!='')
                            { ?>
                                <div class="alert alert-success alert-dismissible">
                                  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                                  <h5><i class="icon fas fa-check"></i> <?php echo $this->session->flashdata('msg_berhasil_upload');?></h5>
                                </div>
                      <?php } ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <td style="text-align: center; vertical-align: middle;">
                                                        ...
                                                        <?php
                                                            if ($item['upload_file_ra'] == NULL) 
                                                            { ?>
                                                                <button onclick="show_modal_upload_ra(<?php echo htmlspecialchars($item['id_aktualisasi']); ?>)" title="Upload File RA" class="btn btn-xs btn-success">
                                                                    <i class="fas fa-upload"></i> <b>RA</b>
                                                                </button>
                                                      <?php }
                                                            else
                                                            { ?>
                                                                <button onclick="show_modal_upload_ra(<?php echo htmlspecialchars($item['id_aktualisasi']); ?>)" title="Upload Ulang File RA" class="btn btn-xs btn-warning">
                                                                    <i class="fas fa-upload"></i> <b>RA</b>
                                                                </button>
                                                      <?php } ?>

                                                        <!--Tombol upload file laporan LA peserta-->
                                                        <?php
                                                            if ($item['upload_file_la'] == NULL) 
                                                            { ?>
                                                                <button onclick="show_modal_upload_la(<?php echo htmlspecialchars($item['id_aktualisasi']); ?>)" title="Upload File LA" class="btn btn-xs btn-success">
                                                                    <i class="fas fa-upload"></i> <b>LA</b>
                                                                </button>
                                                      <?php }
                                                            else
                                                            { ?>
                                                                <button onclick="show_modal_upload_la(<?php echo htmlspecialchars($item['id_aktualisasi']); ?>)" title="Upload Ulang File LA" class="btn btn-xs btn-warning">
                                                                    <i class="fas fa-upload"></i> <b>LA</b>
                                                                </button>
                                                      <?php } ?>

                                                        <!--Tombol hapus data peserta-->
                                                        <a href="<?php echo site_url('admin/Diklat/hapus_peserta_pelatihan_latsar/?id_aktualisasi='.htmlspecialchars($item['id_aktualisasi'])); ?>" onclick="return confirm('Yakin ingin menghapus data peserta pada pelatihan ini?');" title="Hapus Data Peserta" class="btn btn-xs btn-danger">
...
<!--show modal konfirmasi ganti coach ==============================================================================================-->
<div class="modal fade" id="modal-ganti-coach">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            ...
            <form role="form" enctype="multipart/form-data" action="<?php echo site_url('admin/Diklat/ganti_coach_admin/');?>" method="POST">
                <div class="card-body pad">
                    <div class="form-group">
                        <label>Pilih Nama Coach</label>
                        <select required onchange="onchange_ajax_nama_coach()" id="id_coach" name="id_coach" class="form-control" style="width: 100%;">
                          <option disabled selected>--Pilih Nama Coach Baru--</option>
                          <?php
                              foreach ($pakwi as $item) 
                              { ?>
                                  <option value="<?php echo htmlspecialchars($item->nip); ?>"><?php echo htmlspecialchars($item->nama); ?></option>  
                        <?php }
                          ?>
                        </select>
                    </div>
                    ...
                    <input name="nama_coach" id="nama_coach" required hidden readonly>
                </div>
...
<div class="modal fade" id="show-upload-ra">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            ...
            <form role="form" enctype="multipart/form-data" action="<?php echo site_url('admin/Diklat/upload_ra_admin/');?>" method="POST">
                <div class="card-body pad">
                    <div class="form-group">
                        <label>Pilih File RA</label>
                        <input onchange="return validasiFile_ra()" required id="file_upload_ra" style="padding: 3px;" type="file" class="form-control" name="file_upload_ra" required>
                    </div>
                    
                    <input id="id_aktualisasi_upload_ra" value="" name="id_aktualisasi_upload_ra" hidden readonly>
                    <input id="id_diklat_upload" value="<?php echo htmlspecialchars($id_diklat); ?>" name="id_diklat_upload" hidden readonly>
                </div>
                <div class="modal-footer justify-content-between">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
                    <button type="submit" name="simpan" value="1" class="btn btn-success">Simpan</a>
...
<!--show modal upload file LA ======================================================================================================-->
<div class="modal fade" id="show-upload-la">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" style="color:red;">Upload file LA</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form role="form" enctype="multipart/form-data" action="<?php echo site_url('admin/Diklat/upload_la_admin/');?>" method="POST">
                <div class="card-body pad">
                    <div class="form-group">
                        <label>Pilih File LA</label>
                        <input onchange="return validasiFile_la()" required id="file_upload_la" style="padding: 3px;" type="file" class="form-control" name="file_upload_la" required>
                    </div>
                    
                    <input id="id_aktualisasi_upload_la" value="" name="id_aktualisasi_upload_la" hidden readonly>
                    <input id="id_diklat_upload" value="<?php echo htmlspecialchars($id_diklat); ?>" name="id_diklat_upload" hidden readonly>
                </div>
                <div class="modal-footer justify-content-between">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
...
<script type="text/javascript">
    ...
    //fungsi untuk menampilkan modal upload ra
    function show_modal_upload_ra(id_aktualisasi)
    {
        $('#show-upload-ra').modal('show'); //buka modal show upload RA

        $('#id_aktualisasi_upload_ra').val(id_aktualisasi);
    }

    //fungsi untuk menampilkan modal upload LA
    function show_modal_upload_la(id_aktualisasi)
    {
        $('#show-upload-la').modal('show'); //buka modal show upload LA

        $('#id_aktualisasi_upload_la').val(id_aktualisasi);
    }

    function validasiFile_ra()
    {
        var inputFile = document.getElementById('file_upload_ra');
    ...
    function validasiFile_la()
    {
        var inputFile = document.getElementById('file_upload_la');
        var pathFile = inputFile.value;
        var file = inputFile.files[0];
        var size_file = file.size;
        var ekstensiOk = /(\.pdf|\.PDF)$/i;
